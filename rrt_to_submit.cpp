...
void init()
{
    start_node1.position.x = 70;
    start_node1.position.y = 70;
...
temp_node1=end_node1;
while(temp_node1.parent!=NULL)
{
  cout<<"coordinate x: "<<temp_node1.position.x<<"   coordinate y: "<<temp_node1.position.y<<endl;
  refer.x=temp_node1.position.x;
  refer.y=temp_node1.position.y;
  f.write(reinterpret_cast<char*>(&refer),sizeof(reference));
  temp_node1=*(temp_node1.parent);
}

  
cout<<endl<<"x: "<<node1s[totnode1s-1]->position.x<<"  y: "<<node1s[totnode1s-1]->position.y;
f.close();
